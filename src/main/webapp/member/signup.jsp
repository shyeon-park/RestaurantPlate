<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    ...
    <body>
        <!-- id 인풋 모달 Modal -->
        <form id="signupForm" action="${pageContext.request.contextPath}/addMember.mem" method="post">
            <div class="modal fade" id="idCheckModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body d-flex">
                            <input type="text" class="w-75 me-3" id="chickId" name="chickId"
                                placeholder="ID는 6~12글자 영문자로 시작해야만 합니다.">
                            <button type="button" class="form-control w-25" id="sendId">아이디
                                확인</button>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">취소</button>
                            <button type="button" class="btn btn-primary" id="btnModalClose"
                                disabled="disabled">사용하기</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <script type="text/javascript">
                ...
                document.getElementById("sendId").addEventListener("click", function () {
                    if (!idCheck()) return
                    let data = { "chickId": $("#chickId").val() };
                    console.log(data)
                    $.ajax({
                        type: "post"
                        , url: "${pageContext.request.contextPath}/checkId.mem"
                        , data: data
                        , success: function (rs) {
                            console.log(rs)
                            ...
                            let newP
                            if (rs == "success") {
                                newP = "<div id='resultId'><p class = 'ms-3'style='color: green'>사용가능한 아이디입니다.</p></div>"
                                $("#btnModalClose").removeAttr("disabled")
                            } else {
                                newP = "<div id='resultId'><p class = 'ms-3'style='color: red'>중복된 아이디입니다.</p></div>"
                                $("#btnModalClose").attr("disabled", true)
                                //$("#chickId").val("")
                            }
                            $(".modal-body").after(newP)
                ...
                //id 정규식 
                //영문자로 시작 영어숫자 6자리이상 12자리 이하
                function idCheck() {
                    let regex = /^[a-zA-Z][a-zA-Z0-9_]{5,11}$/
                    let chickId = document.getElementById("chickId")
                    if (!check(regex, chickId.value, chickId, "올바르지 않은 Id 형식입니다.")) {
                        return false;
                    }
                    return true;
                }
        ...
        <!--다음 POST CODE API CDN-->
        <script src="//t1.daumcdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js"></script> 

        <script>
       	    let id = document.getElementById("id") // id 인풋창
            let pw = document.getElementById("pw") //패스워드 인풋창
            let pwCheck = document.getElementById("pwCheck") // 패스워드확인 인풋창
            ...
            let idRegex = /^[a-zA-Z][a-zA-Z0-9_]{5,11}$/
            ...
            pw.addEventListener("blur", function () {
                $(".resultPw").remove();
                let newP
                if (regexTest(pwRegex, pw)) {
                    //newP = "<div class = 'col-12 resultPw d-flex justify-content-start mt-3'><p class = 'm-0 ms-3'style='color: green; font-size: 0.8rem;' >사용가능한 비밀 번호 입니다.</p></div>"    
                    newP = "<div class ='resultPw'></div>"
                } else {
                    newP = "<div class ='col-12 resultPw d-flex justify-content-start mt-3'><p class = 'm-0 ms-3'style='color: red; font-size: 0.8rem;' >비밀번호는 8자리 이상 16자리 이하</p></div><div class = 'col-12 resultPw d-flex justify-content-start' id=''><p class = 'm-0 ms-3'style='color: red; font-size: 0.8rem;'>영문자 숫자 특수문자가 각각 1개 이상 포함 되어야 합니다.</p></div>"
                }
                $("#pw").after(newP)
            ...
            pwCheck.addEventListener("keyup", function () {
                $("#pwCheckText").remove();
                if (checkPassword() == "불일치") newP = "<div class ='mt-3 d-flex justify-content-start' id='pwCheckText'> <p class = 'm-0 ms-3'style='color: red; font-size: 0.8rem;' >비밀번호가 일치하지 않습니다</p></div>"
                else if (checkPassword() == "빈칸") newP = "<div class ='mt-3 d-flex justify-content-start' id='pwCheckText'> <p class = 'm-0 ms-3'style='color: red; font-size: 0.8rem;' >필수 입력 사항입니다.</p></div>"
			...
			btnSubmit.addEventListener("click",function(){
				...
				if(!regexTest(idRegex,id)) return id.focus()
