...
public class RestaurantDAO {
	...
	public ArrayList<RestaurantJoinFileDTO> selectRestAndFileBySeq(int listNum) throws Exception {
		...
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
			...
			while (rs.next()) {
				...
				list.add(new RestaurantJoinFileDTO(seq_rest, seq_list, rest_name, rest_introduction, sido, sigungu,
						bname, postCode, rest_address, rest_tel, rest_time, parking_possible, system_name));
			}
			return list;
		}
	}
	...
	public RestaurantDTO selectBySeq_rest(int seq_rest) throws Exception {
		...
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
			...
			if (rs.next()) {
				...
				return new RestaurantDTO(seq_rest, seq_list, rest_name, rest_introduction, sido, sigungu, bname,
						postCode, rest_address, rest_tel, rest_time, parking_possible, mark_count);
			}
		}
		return null;
	}
	...
	// 맛집 삭제
	public int deleteBySeq(int seq_rest) throws Exception {
		String sql = "DELETE FROM tbl_rest WHERE seq_rest=?";
